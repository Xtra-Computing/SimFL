...
//                        min_fea.resize(i);
//                        max_fea.resize(i);
//                        min_fea[i - 1] = INFINITY;
//                        max_fea[i - 1] = -INFINITY;
                    }
...
//        this->line_num.resize(n_features_);
        this->features.resize(max_dimension);
        this->line_num.resize(max_dimension);
        for(int i = 0; i < nthread; i++) {
    ...
    n_features_ = max_dimension;
    min_fea.resize(max_dimension);
    max_fea.resize(max_dimension);
    for(int i = 0; i < max_dimension; i++){
        if(features[i].size() == 0){
            ...
            min_fea[i] = *std::min_element(features[i].begin(), features[i].end());
            max_fea[i] = *std::max_element(features[i].begin(), features[i].end());
        }
    }
    LOG(INFO) << "#instances = " << this->n_instances() << ", #features = " << this->n_features();
}

void DataSet::load_from_file_csr(string file_name) {
    LOG(INFO) << "loading LIBSVM dataset from file \"" << file_name << "\"";
    y_.clear();
    instances_.clear();
    n_features_ = 0;
...
void DataSet::load_from_file_two_dimension(string file_name){
    LOG(INFO) << "loading LIBSVM dataset from file \"" << file_name << "\"";
    y_.clear();
    features.clear();
    line_num.clear();

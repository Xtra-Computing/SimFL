...
void ExactUpdater::lsh_hash_init(unsigned n_bucket, unsigned num_table, unsigned num_dimension, unsigned p_norm, float r, unsigned numP, int seed){
    lsh_table.param.n_bucket = n_bucket;
    lsh_table.param.n_table = num_table;
    lsh_table.param.n_dimension = num_dimension;
    lsh_table.param.p_norm = p_norm;
    lsh_table.param.r = r;
    lsh_table.param.n_comp = numP;
    lsh_table.param.seed = seed;
    lsh_table.init();
}
...
void ExactUpdater::grow(Tree &tree, const vector<std::shared_ptr<SparseColumns>> &v_columns, InsStat &stats) {
    ...
    int cur_device = 0;
//    int cur_device = param.use_gpu_id;
